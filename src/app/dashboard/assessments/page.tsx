...
export default function AssessmentsPage() {
  ...
  useEffect(() => {
    ...
    if (compareData != null && isCompareModalVisible === true) {
      ...
      setTimeout(() => {
        ...
        if (
          avgContainer &&
          avgContainer.children.length === 0 &&
          avgContainer.clientWidth > 0
        ) {
      ...
      setTimeout(() => {
        ...
        if (
          scoreContainer &&
          scoreContainer.children.length === 0 &&
          scoreContainer.clientWidth > 0
        ) {
  ...
  const columns: ProColumns<AssessmentRecord>[] = [
    ...
    },
    {
      ...
      render: (_, record) => [
        ...
        <a
          key="compare1"
          onClick={() => handleCompare("1", record.id)}
          style={{ marginLeft: 8 }}
        >
          和上次作答比较
        </a>,
        <a
          key="compare2"
          onClick={() => handleCompare("2", record.id)}
          style={{ marginLeft: 8 }}
        >
          和首次作答比较
        </a>,
  ...
  const initCharts = (data: AssessmentResult[]) => {
    ...
    const columnData = data
      .filter(
        (item) => !["不能社交", "学习障碍", "情绪障碍"].includes(item.category)
      )
      .map((item) => {
        console.log(item);
        // 排除特定维度的等级计算
        ...
        // 处理萌芽等级字符串，取最大值
        const potentialLevels = item.potentialLevel
          .split(",")
    ...
    try {
      ...
      if (currentResult) {
        const response: any = await api.post(
          "/admin/assessment/updateInterventionSuggestion",
          {
            id: currentResult.id,
        ...
        if (response.success) {
          ...
          // 刷新详情数据
          if (detailData) {
            const updatedResponse: any = await api.get(
          ...
          <div className="print-content" style={{ padding: "24px" }}>
            ...
            <Descriptions bordered column={2}>
              ...
              <Descriptions.Item label="儿童姓名">
                {detailData.chileName}
              </Descriptions.Item>
              <Descriptions.Item label="年龄">
                {detailData.chileAge} 岁
              </Descriptions.Item>
              ...
              <Descriptions.Item label="开始答题时间">
